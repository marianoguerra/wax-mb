...
# see moon.work.
#
# List the files the four published modules would ship.
[group('ship')]
package:
    moon -C lib package --list
    moon -C cli package --list
    moon -C was package --list
...
publish-dry: ci
    ...
    tools/publish-dry.sh was
    tools/publish-dry.sh wap

# The library first: wax-cli, was and wap all pin a version of it that must
# already exist on the registry. Refresh the registry after publishing the library so
...
#
# Publish all four modules to mooncakes.io, after the full CI gate. Not reversible.
[group('ship')]
publish: ci
    moon -C lib publish
    moon update
